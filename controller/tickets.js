  ...
  try {
    tickets.find((err, tickets) => {
      if (err) {
        ...
        const copyTickets = [...tickets].map((item) => {
          item.ticketStatus = item.ticketStatus.toLowerCase();
          item.ticketPriority = item.ticketPriority.toLowerCase();
          return item;
        });
        res.status(200).json(copyTickets);
      }
    });
  } catch (error) {
    return res.status(400).json({
...
module.exports.processEdit = (req, res, next) => {
  try {
    ...
    let updatedItem = tickets({
      ...
      ticketStatus: req.body.ticketStatus.toLowerCase(),
      ticketDescription: req.body.ticketDescription,
      ticketPriority: req.body.ticketPriority.toLowerCase(),
    });
...
module.exports.processAdd = (req, res, next) => {
  try {
    let createTicket = tickets({
      //create new ticket object with all proprties in model
